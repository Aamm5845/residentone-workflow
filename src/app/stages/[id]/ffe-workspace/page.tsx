...
export default async function FFEWorkspacePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    stage = await prisma.stage.findFirst({
      where: { 
        ...
        type: 'FFE' // Only allow FFE workspace for FFE stages
      },
      select: {
        id: true,
        type: true,
  ...
  // Redirect to the new dedicated FFE workspace page
  redirect(`/ffe/${stage.room.id}/workspace`)
}
