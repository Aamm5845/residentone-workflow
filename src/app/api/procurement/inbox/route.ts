 ...
 * Get inbox notifications across ALL projects for the current org
 * Returns: { projects: [{ projectId, projectName, items }], totalCount }
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const now = new Date()
    const projectResults: { projectId: string; projectName: string; items: any[] }[] = []
    let totalCount = 0
    ...
    for (const project of projects) {
      ...
      if (rfqIds.length > 0) {
        ...
        for (const quote of supplierQuotes) {
          ...
          const hasMismatches = quote.lineItems.some(li =>
            li.alternateProduct ||
            (li.quantity !== li.rfqLineItemId ? false : false)
          )
      ...
      // 2a. Client viewed invoice but hasn't paid
    ...
    return NextResponse.json({ projects: projectResults, totalCount })
