...
const OLD_FOLDER_NAME = '2- MAX'
const NEW_FOLDER_NAME = '2- 3D Models'

/**
 * POST /api/admin/rename-max-folders
 * Rename "2- MAX" to "2- 3D Models" in all existing project Dropbox folders.
 * If the old folder doesn't exist, create "2- 3D Models" instead.
 *
 * Body:
...
export async function POST(request: NextRequest) {
  try {
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            if (createMsg.includes('conflict') || createMsg.includes('already exists') || createMsg.includes('409')) {
              results.push({
                projectId: project.id,
                projectName: project.name,
                dropboxFolder,
                status: 'existed',
                reason: `${NEW_FOLDER_NAME} folder already exists`,
              })
            } else {
        ...
        } else if (renameMsg.includes('conflict') || renameMsg.includes('409')) {
    ...
    console.error('[rename-max-folders] Error:', error)
    return NextResponse.json(
