...
function Header({ org, logoDataUri }: { org: any; logoDataUri: string | null }) {
  return React.createElement(View, { style: styles.header },
    ...
    logoDataUri
      ...
      : React.createElement(View, { style: { ...styles.logoBox, backgroundColor: colors.headerBg, padding: 10 } },
  ...
  try {
    ...
    return new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="${proposal.proposalNumber}-${proposal.clientName.replace(/\s+/g, '-')}.pdf"`,
      },
    })
  } catch (error) {
